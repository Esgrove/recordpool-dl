...
`RecordPool` is an abstract base class that provides the common interface, which is inherited by all recordpool-specific classes.
`Bandcamp, Beatjunkies, BPMSupreme, DJCity` are sub-classes of `RecordPool` that implement the common functions,
like *get_tracks* and *download_track* for the respective website (if a custom implementation is required).
